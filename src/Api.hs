...
data Game = Game {player1 :: Player, player2 :: Player, table :: [Card], pot :: Int} deriving(Generic, Show)
...
main = do
    ...
    scotty 3000 $ do
        ...
        get "/pick" $ do
            ...
            g <- liftIO var
            c <- liftIO $ pick (table g ++ destroyTuple (cards $ player1 g) ++ destroyTuple (cards $ player2 g))
            json c
